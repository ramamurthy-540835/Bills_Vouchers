...
class FinanceRepository:
    ...
    def documents(self,client_id,limit=100,offset=0,include_extraction=False):
        ...
        for r in rows:
            d=ns(**self.d(r)); d.document_type=ns(value=d.document_type); d.status=ns(value=d.status); d.extraction=self.extraction(d.id) if include_extraction else None; out.append(d)
        return out
