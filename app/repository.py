...
class FinanceRepository:
    ...
    def user_by_id(self, uid):
        if not uid:
            return None
        r = self.bq.one(
            f"SELECT * FROM `{self.bq.table('users')}` WHERE id=@id AND is_active=TRUE LIMIT 1",
            [bigquery.ScalarQueryParameter("id", "STRING", str(uid))],
        )
        return ns(**self.d(r)) if r else None

    def user_by_email(self, email):
        r = self.bq.one(
            f"SELECT * FROM `{self.bq.table('users')}` WHERE email=@email LIMIT 1",
    ...
    def clients(self, user_id=None):
        if user_id:
            ...
            return [
                ns(**self.d(r))
                for r in self.bq.query(sql, [bigquery.ScalarQueryParameter("uid", "STRING", str(user_id))])
            ]
        return [
            ns(**self.d(r))
            for r in self.bq.query(f"SELECT * FROM `{self.bq.table('clients')}` WHERE is_active=TRUE ORDER BY name")
        ]

    def client(self, client_id):
        r = self.bq.one(
            f"SELECT * FROM `{self.bq.table('clients')}` WHERE id=@id AND is_active=TRUE",
            [bigquery.ScalarQueryParameter("id", "STRING", str(client_id))],
        )
        return ns(**self.d(r)) if r else None
    ...
    def can_access_client(self, user_id, client_id):
        return bool(
            self.bq.one(
                f"SELECT 1 FROM `{self.bq.table('client_memberships')}` WHERE user_id=@u AND client_id=@c AND is_active=TRUE LIMIT 1",
                [
                    bigquery.ScalarQueryParameter("u", "STRING", str(user_id)),
                    bigquery.ScalarQueryParameter("c", "STRING", str(client_id)),
                ],
            )
        )
    ...
    def _account(self, r):
        a = ns(**self.d(r))
        a.account_type = ns(value=a.account_type)
        return a

    def account(self, aid, client_id=None):
        params = [bigquery.ScalarQueryParameter("id", "STRING", str(aid))]
        scope = ""
        if client_id:
            scope = " AND client_id=@client"
            params.append(bigquery.ScalarQueryParameter("client", "STRING", str(client_id)))
        r = self.bq.one(f"SELECT * FROM `{self.bq.table('accounts')}` WHERE id=@id{scope}", params)
        return ns(**self.d(r)) if r else None

    def documents(self, client_id, limit=100, offset=0, include_extraction=False):
        rows = self.bq.query(
            f"SELECT * FROM `{self.bq.table('documents')}` WHERE client_id=@client ORDER BY uploaded_at DESC LIMIT @n OFFSET @offset",
            [
                bigquery.ScalarQueryParameter("client", "STRING", str(client_id)),
                bigquery.ScalarQueryParameter("n", "INT64", limit),
                bigquery.ScalarQueryParameter("offset", "INT64", offset),
            ],
        )
        out = []
        for r in rows:
            d = ns(**self.d(r))
            d.document_type = ns(value=d.document_type)
            d.status = ns(value=d.status)
            d.extraction = self.extraction(d.id) if include_extraction else None
            out.append(d)
        return out
    ...
    def extraction(self, did):
        ...
        e.line_items = [ns(**self.d(x)) for x in rows]
        return e

    def recent_entries(self, client_id, limit=8):
        return [
            ns(**self.d(r))
            for r in self.bq.query(
                f"SELECT * FROM `{self.bq.table('journal_entries')}` WHERE client_id=@client ORDER BY created_at DESC LIMIT @n",
                [
                    bigquery.ScalarQueryParameter("client", "STRING", str(client_id)),
                    bigquery.ScalarQueryParameter("n", "INT64", limit),
                ],
    ...
    def audit_logs(self, client_id, limit=50, offset=0):
        return [
            ns(**self.d(r))
            for r in self.bq.query(
                f"SELECT * FROM `{self.bq.table('audit_logs')}` WHERE client_id=@client ORDER BY created_at DESC LIMIT @n OFFSET @offset",
                [
                    bigquery.ScalarQueryParameter("client", "STRING", str(client_id)),
                    bigquery.ScalarQueryParameter("n", "INT64", limit),
                    bigquery.ScalarQueryParameter("offset", "INT64", offset),
