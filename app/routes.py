...
def signup(request:Request,email:str=Form(...),full_name:str=Form(...),password:str=Form(...),repo=Depends(get_db)):
    ...
    request.session['user_id']=user_id; fr(repo).audit(user_id,'signup','user',user_id)
    return RedirectResponse('/',303)
@router.post('/logout')
def logout(request:Request): request.session.clear(); return RedirectResponse('/login',303)
@router.get('/')
def dashboard(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    accounts=fr(repo).accounts(); entries=fr(repo).recent_entries()
    return page('dashboard.html',request,user=user,accounts=accounts,balances={account.id:0 for account in accounts},recent=entries)
@router.get('/accounts')
def accounts(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    rows=fr(repo).accounts(); return page('accounts.html',request,user=user,accounts=rows,balances={account.id:0 for account in rows},types=list(AccountType))
...
def transactions(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    return page('transactions.html',request,user=user,accounts=fr(repo).accounts(True),entries=fr(repo).recent_entries(100))
...
def expenses(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    accounts=fr(repo).accounts(True)
    rows=repo.query(f"SELECT * FROM `{repo.table('journal_entries')}` WHERE source='expense' ORDER BY created_at DESC LIMIT 100")
    return page('expenses.html',request,user=user,accounts=accounts,entries=rows)
...
def create_expense(reference:str=Form(...),description:str=Form(...),amount:str=Form(...),expense_account_id:str=Form(...),payment_account_id:str=Form(...),entry_date:date=Form(...),repo=Depends(get_db),user=Depends(current_user)):
    ...
    repo.insert('journal_entries',{'id':eid,'entry_date':entry_date.isoformat(),'reference':reference.strip(),'description':description.strip(),'source':'expense','status':'posted','created_at':now},eid)
    ...
    fr(repo).audit(user.id,'post','expense',eid); return RedirectResponse('/expenses',303)
@router.get('/documents/upload')
def upload_document_page(request:Request,user=Depends(current_user)): return page('document_upload.html',request,user=user,types=list(DocumentType))
...
async def upload_document(document_type:DocumentType=Form(...),file:UploadFile=File(...),repo=Depends(get_db),user=Depends(current_user)):
    if user.role=='viewer': raise HTTPException(403,'Viewer access is read-only.')
    payload,filename,mime=await validate_upload(file); d=create_document(fr(repo),user,document_type,filename,mime,payload,GCSObjectStore(get_settings().gcs_bucket_name))
...
def documents(request:Request,repo=Depends(get_db),user=Depends(current_user)): return page('documents.html',request,user=user,documents=fr(repo).documents())
...
def review_document(document_id:str,request:Request,repo=Depends(get_db),user=Depends(current_user)):
    d=fr(repo).document(document_id)
    if not d: raise HTTPException(404,'Document not found.')
    return page('document_review.html',request,user=user,document=d,extraction=d.extraction)
...
def reports(request:Request,start:date|None=None,end:date|None=None,repo=Depends(get_db),user=Depends(current_user)):
    ...
    return page('reports.html',request,user=user,start=start,end=end,data=data,pnl=pnl,revenue=revenue,expenses=expenses,profit=profit,cash=cash)
...
def audit_logs(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    ...
    return page('audit_logs.html',request,user=user,rows=fr(repo).audit_logs())
@router.get('/users')
def users(request:Request,repo=Depends(get_db),user=Depends(current_user)):
    if user.role!='admin': raise HTTPException(403,'Admin access is required.')
    rows=repo.query(f"SELECT id,email,full_name,role,is_active,created_at FROM `{repo.table('users')}` ORDER BY created_at DESC")
    return page('users.html',request,user=user,users=rows,roles=list(Role))
...
def update_user(user_id:str,role:Role=Form(...),is_active:bool=Form(False),repo=Depends(get_db),user=Depends(current_user)):
    ...
    fr(repo).audit(user.id,'update','user',user_id); return RedirectResponse('/users',303)
@router.get('/settings')
def settings_page(request:Request,user=Depends(current_user)): return page('settings.html',request,user=user)
...
def update_settings(request:Request,full_name:str=Form(...),new_password:str=Form(''),repo=Depends(get_db),user=Depends(current_user)):
    ...
    fr(repo).audit(user.id,'update','profile',user.id); return RedirectResponse('/settings',303)
def _doc_json(d):
    e=d.extraction
