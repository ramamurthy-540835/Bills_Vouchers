...
def process_with_gemini(repo,document,payload):
    repo.bq.update('documents',"status='processing', processing_error=NULL",'id=@id',[bigquery.ScalarQueryParameter('id','STRING',document.id)])
    try: data=scan_document(document,payload)
    except Exception as exc:
        repo.bq.update('documents',"status='failed', processing_error=@err",'id=@id',[bigquery.ScalarQueryParameter('id','STRING',document.id),bigquery.ScalarQueryParameter('err','STRING',str(exc)[:2000])]); raise
    def dt(x):
        try: return date.fromisoformat(str(x)[:10]).isoformat() if x else None
        except ValueError: return None
    row={'id':document.id,'document_id':document.id,**{k:data.get(k) for k in ['vendor_name','vendor_address','invoice_number','gstin','currency','payment_method','ocr_text']},'invoice_date':dt(data.get('invoice_date')),'due_date':dt(data.get('due_date')),'subtotal':decimal_or_none(data.get('subtotal')),'tax_amount':decimal_or_none(data.get('tax_amount')),'cgst':decimal_or_none(data.get('cgst')),'sgst':decimal_or_none(data.get('sgst')),'igst':decimal_or_none(data.get('igst')),'discount_amount':decimal_or_none(data.get('discount_amount')),'total_amount':decimal_or_none(data.get('total_amount')),'ocr_confidence':None,'created_at':str(document.uploaded_at)}
    ...
    for i,x in enumerate(data.get('line_items') or []): repo.bq.insert('document_line_items',{'id':f'{document.id}-{i}','extraction_id':document.id,'line_number':i,**{k:x.get(k) for k in ['item_name','description','unit']},'quantity':decimal_or_none(x.get('quantity')),'unit_price':decimal_or_none(x.get('unit_price')),'tax':decimal_or_none(x.get('tax')),'discount':decimal_or_none(x.get('discount')),'total':decimal_or_none(x.get('total'))},f'{document.id}-{i}')
    repo.bq.update('documents',"status='needs_review', processing_error=NULL",'id=@id',[bigquery.ScalarQueryParameter('id','STRING',document.id)])
