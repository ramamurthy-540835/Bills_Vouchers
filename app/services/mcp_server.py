...
def powerbi_dashboard_data(client_id:str='client_demo_001')->list[dict]:
    ...
    repo=get_repository()
    rows=repo.query(f'''SELECT month,category,income,expense FROM `{repo.table("powerbi_finance_dashboard")}` WHERE client_id=@client ORDER BY month,category''',[bigquery.ScalarQueryParameter('client','STRING',client_id)])
