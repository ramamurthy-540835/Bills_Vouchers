...
2. Copy `infra/terraform.tfvars.example`, complete it with immutable image digests/tags, then run `terraform init` and review `terraform plan` from `infra`. Apply only from an approved change window.
3. Replace `PROJECT_ID` in `infra/vector_search.sql` and run it in BigQuery to create the IVF vector index.
4. Configure `GCP_PROJECT_ID`, `GCP_REGION=global`, `GCS_BUCKET_NAME`, `BIGQUERY_DATASET=finance_analytics`, `GEMINI_MODEL=gemini-2.5-flash`, and `EMBEDDING_MODEL=gemini-embedding-001` as Cloud Run environment variables.
...
6. Build with `gcloud builds submit --config cloudbuild.yaml --substitutions=_REGION=asia-south1`, then pass the resulting SHA-tagged backend and frontend images to Terraform. Cloud Run resources are configured for 1 CPU/1Gi backend and 1 CPU/512Mi frontend with bounded concurrency.
