...
export default function Login() {
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{const r=await fetch('/api/auth/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,password})});const text=await r.text();let data:any={};try{data=text?JSON.parse(text):{}}catch{}if(!r.ok){setError(data.detail||data.message||'Login failed ('+r.status+')');return}router.push(data.must_change_password?'/settings':'/')}catch{setError('Finance backend is unavailable. Start the API and try again.')}finally{setBusy(false)}}
